...
export function AmbientDevSwitcher() {
  ...
  return (
    <aside
      ...
      className="absolute bottom-5 right-5 z-20 flex items-center gap-1 rounded-full border border-white/10 bg-[#0d0e19]/80 p-1.5 shadow-2xl backdrop-blur-xl transition-opacity hover:opacity-100"
    >
      <span className="px-2.5 text-[10px] font-medium tracking-[0.16em] uppercase text-violet-200/50">
        Mood
      </span>
        ...
        {AMBIENT_MOOD_OPTIONS.map((option) => {
          ...
          return (
            <button
              ...
              className={`relative rounded-full px-2.5 py-1 text-xs font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-300 ${
                isActive
                  ? 'text-white'
